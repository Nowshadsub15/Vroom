...
void car_draw(Car *car)
{
    ...
    Rectangle destBack = { car->back_wheel.position.x, car->back_wheel.position.y, back_diameter, back_diameter }; 
    Vector2 originBack = { car->back_wheel.radius, car->back_wheel.radius };
    
    DrawTexturePro(car->back_wheel.texture, sourceBack, destBack, originBack,car->back_wheel.angle, WHITE);
    ...
    Rectangle destFront = { car->front_wheel.position.x, car->front_wheel.position.y, front_diameter, front_diameter };
    Vector2 originFront = { car->front_wheel.radius, car->front_wheel.radius };
    
    DrawTexturePro(car->front_wheel.texture, sourceFront, destFront, originFront, car->front_wheel.angle, WHITE);
}