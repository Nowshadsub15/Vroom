...
bool menu_update_and_draw(Menu *menu, int window_width, int window_height)
{
    ...
    Rectangle button = {window_width / 2.0f - 110, window_height / 2.0f - 5, 220, 70};
    ...
    DrawTextEx(menu->font, title, (Vector2){window_width / 2.0f - title_size.x, window_height / 3.5f}, title_font_size, 0, DARKBLUE); // co ordinates using ai
    ...
    float hint_font_size = 24.0f;
    Vector2 hint_size = MeasureTextEx(menu->font, hint, hint_font_size, 0);
    DrawTextEx(menu->font, hint, (Vector2){window_width / 2.0f - hint_size.x / 2.0f, button.y + button.height + 30}, hint_font_size, 0, DARKGRAY);
